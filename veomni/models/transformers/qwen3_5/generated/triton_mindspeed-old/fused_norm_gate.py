...
def layer_norm_gated_fwd_kernel(
    ...
    for offset in range(0, bt_size):
        ...
        if block_start < T:
            ...
            if HAS_RESIDUAL:
                p_res = tl.make_block_ptr(residual, (T, D), (D, 1), (block_start, 0), (BT, BD), (1, 0))
                b_x += tl.load(p_res, boundary_check=(0, 1)).to(tl.float32)
            if STORE_RESIDUAL_OUT:
                p_res_out = tl.make_block_ptr(residual_out, (T, D), (D, 1), (block_start, 0), (BT, BD), (1, 0))
                tl.store(p_res_out, b_x.to(p_res_out.dtype.element_ty), boundary_check=(0, 1))
            if not IS_RMS_NORM:
                b_mean = tl.sum(b_x, axis=1) / D
                p_mean = tl.make_block_ptr(mean, (T,), (1,), (block_start,), (BT,), (0,))
            ...
            p_g = tl.make_block_ptr(g, (T, D), (D, 1), (block_start, 0), (BT, BD), (1, 0))
