        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            for i, (cached_args, cached_kwargs, cached_result) in enumerate(cache):
                if len(args) == len(cached_args) and len(kwargs) == len(cached_kwargs):
                    ...
                    #     print("_is_match(a, b) for a, b in zip(args, cached_args):", [_is_match(a, b) for a, b in zip(args, cached_args)])
                    #     print("_is_match(v, cached_kwargs[k]) for k, v in kwargs.items():", [_is_match(v, cached_kwargs[k]) for k, v in kwargs.items()])
                    #     import traceback
...
def input_guard(make_contiguous: bool = True, auto_to_device: bool = True):
    """Decorator that ensures tensor arguments are contiguous and on the correct device."""
    def decorator(fn):
        @functools.wraps(fn)
        def wrapper(*args, **kwargs):
            ...
            def _process(t):
                if not isinstance(t, torch.Tensor):
                    return t
                if auto_to_device and device is not None:
                    t = t.to(device)
                if make_contiguous:
                    t = t.contiguous()
                return t

            new_args = [_process(a) for a in args]
            new_kwargs = {k: _process(v) for k, v in kwargs.items()}
...
def prepare_chunk_indices(cu_seqlens: torch.Tensor, chunk_size: int) -> torch.Tensor:
    """
    Given cumulative sequence lengths and a chunk size, build an int32 tensor
    of shape [num_total_chunks, 2] where each row is [seq_idx, chunk_idx].

    The tensor is 2-D so that ``len(chunk_indices) == num_total_chunks``,
    which the caller uses directly as NUM_CHKS.  The triton kernel accesses it
    as a flat pointer: ``chunk_indices + i_chk * 2`` gives the seq_idx element
    and ``chunk_indices + i_chk * 2 + 1`` gives the chunk_idx element — which
    matches the C-contiguous [num_chunks, 2] memory layout.
    """
    ...
    for i in range(N):
        ...
        for j in range(num_chunks):
            pairs.append([i, j])
    if not pairs:
        return torch.zeros(0, 2, dtype=torch.int32, device=cu_seqlens.device)
...
def causal_conv1d_bwd_kernel(
    ...
    for task_id in range(pid, total_tasks, num_programs):
        ...
        # Tail-of-allocation guard: if the block's absolute end row exceeds
        # TOTAL_ROWS (= B * T), MTE DMA would read past the packed tensor's
        # allocation and trigger AIC "DDR address out of range". Use BT*W as
        # conservative upper bound covering all three sub-branches below.
            ...
            if USE_INITIAL_STATE:
                for i_w in tl.static_range(1, W):
                    ...
                    for i_t2 in tl.static_range(0, W - 1):
                        if i_t2 < i_w:
                            dy0_row = tl.load(
                                dy + bos * D + (i_t * BT + i_t2) * D + o_d,
                                mask=m_d, other=0.0
                            ).to(tl.float32)
                            if ACTIVATION == "swish" or ACTIVATION == "silu":
                                y0_row = tl.load(
                                    y + bos * D + (i_t * BT + i_t2) * D + o_d,
                                    mask=m_d, other=0.0
                                ).to(tl.float32)
        ...
        if USE_FINAL_STATE:
            if i_t * BT + BT >= T_len - W:
                ...
                start_tok = tl.maximum(0, T_len - W)
                offset = i_t * BT + tl.arange(0, BT)
                w_idx = offset - start_tok           # w_idx ∈ [0, W) 对应 dht 的 W 维
                m_t_fs = (offset >= start_tok) & (offset < T_len) & (w_idx >= 0) & (w_idx < W)
                w_idx_safe = tl.maximum(tl.zeros_like(w_idx), tl.minimum(w_idx, W - 1))
                dht_off = i_n * D * W + o_d[None, :] * W + w_idx_safe[:, None]
                b_dht_raw = tl.load(dht + dht_off, mask=m_d[None, :], other=0.0).to(tl.float32)
                b_dht = tl.where(m_t_fs[:, None] & m_d[None, :], b_dht_raw, tl.zeros_like(b_dht_raw))
                b_dx += b_dht
